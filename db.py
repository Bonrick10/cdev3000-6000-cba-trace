...
def get_transaction_surrounding_info(transaction): 
    # Gets surrounding info of a transaction e.g. entities, branches, accounts merchant tags, device sessions
    ...
    cur.execute("""
        ...
        FROM
            ...
            LEFT JOIN transactions ON (transactions.sender_bsb = %(sender_bsb)s AND transactions.sender_account_number = %(sender_account_number)s)
        WHERE 
            device_sessions.id = %(session_id)s
        ORDER BY 
            transactions.transaction_time
        LIMIT 1
    """, collections.defaultdict(lambda: None, transaction)) # defaultdict convert non entries into None - especially for merchant_tags which may not be provided 
...
print(get_transaction_surrounding_info({
    ...
    "amount": 10,
    # "transaction_time": "2026-01-01 00:00:00+00", 
