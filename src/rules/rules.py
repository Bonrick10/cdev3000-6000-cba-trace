...
def check_rules(transaction, dryrun_flag = True):
    ...
    if check_impossible_travel(transaction, surrounding_info) or check_merchant_type_suspicious_range(transaction, surrounding_info):
...
def check_unseen_device(transaction, surrounding_info, db, dryrun_flag = True): 
    ...
    if not surrounding_info["device_seen_before"] and not dryrun_flag:
        ...
        db.execute("""
            INSERT INTO device_sessions (entity_id, device_id, session_start_time, session_end_time)
            VALUES (%s, %s, %s, NULL)
        """, [surrounding_info["entity_id"], transaction["device_id"], transaction["transaction_time"]])
...
def check_exceed_weekly_total(transaction, surrounding_info):
    ...
    return transaction["amount"] + surrounding_info["24_hour_spending"] > surrounding_info["7_day_spending"]
...
def check_impossible_travel(transaction, surrounding_info): 
    ...
    if surrounding_info["last_transaction_time"] is None: return False
    ...
    delta_time_hours = (datetime.datetime.fromisoformat(transaction["transaction_time"]) - datetime.datetime.fromisoformat(surrounding_info["last_transaction_time"])).total_seconds() / 3600
    
    # Should not happen but avoid division by zero 
    if delta_time_hours == 0: return False 
...
def check_merchant_type_unusual_range(transaction, surrounding_info): 
    ...
    if surrounding_info["usual_threshold_lower"] and transaction["amount"] < surrounding_info["usual_threshold_lower"]:
        return True
    if surrounding_info["usual_threshold_upper"] and transaction["amount"] > surrounding_info["usual_threshold_upper"]:
        return True
    return False 
...
def check_merchant_type_suspicious_range(transaction, surrounding_info): 
    ...
    # Note that lower and upper thresholds may be NULL
    if surrounding_info["suspicious_threshold_lower"] and transaction["amount"] < surrounding_info["suspicious_threshold_lower"]:
        return True
    if surrounding_info["suspicious_threshold_upper"] and transaction["amount"] > surrounding_info["suspicious_threshold_upper"]:
        return True
    return False 