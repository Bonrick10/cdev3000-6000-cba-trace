...
def risk_label(statistic: Dict[str, Any]) -> str:
    ...
    frauds = int(statistic["confirmed_fraud_count"])
    ...
    if smoothed >= SMALL_MODEL_SUSPICIOUS_RATE and lower >= SMALL_MODEL_UNUSUAL_RATE:
...
def calculate_cluster_statistics(
    ...
    assigned["is_confirmed_fraud"] = (
        assigned["original_label"] == Label.CONFIRMED_FRAUDULENT.value
    )
    output = []
    for cluster_id in range(number_of_clusters):
        ...
        fraud_count = int(neighbourhood["is_confirmed_fraud"].sum())
        ...
        statistic = {
            ...
            "confirmed_fraud_percentage": round(raw_rate * 100, 4),
